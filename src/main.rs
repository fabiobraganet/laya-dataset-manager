...
#[derive(Serialize)] struct DatasetExampleSummary { example_id:String, validation_status:String, source:String, updated_at:String }
...
async fn dataset_workspace(State(state): State<AppState>, Path(dataset_id): Path<i64>) -> ApiResult<DatasetWorkspace>{let result=with_db(&state,|db|{let dataset=db.query_row("SELECT id,name,description,created_at,updated_at FROM datasets WHERE id=?1",[dataset_id],|r|Ok(Dataset{id:r.get(0)?,name:r.get(1)?,description:r.get(2)?,created_at:r.get(3)?,updated_at:r.get(4)?}))?;let mut e=db.prepare("SELECT example_id,validation_status,source,updated_at FROM dataset_examples WHERE dataset_id=?1 ORDER BY example_id")?;let examples=e.query_map([dataset_id],|r|Ok(DatasetExampleSummary{example_id:r.get(0)?,validation_status:r.get(1)?,source:r.get(2)?,updated_at:r.get(3)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;let mut v=db.prepare("SELECT id,dataset_id,number,example_count,content_sha256,contract_version,created_at FROM dataset_versions WHERE dataset_id=?1 ORDER BY number DESC")?;let versions=v.query_map([dataset_id],|r|Ok(DatasetVersion{id:r.get(0)?,dataset_id:r.get(1)?,number:r.get(2)?,example_count:r.get(3)?,content_sha256:r.get(4)?,contract_version:r.get(5)?,created_at:r.get(6)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;Ok(DatasetWorkspace{dataset,examples,versions})});result.map(Json).map_err(|_|error(StatusCode::NOT_FOUND,"Dataset não encontrado."))}
